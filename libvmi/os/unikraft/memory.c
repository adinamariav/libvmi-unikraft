...
__vaddr_t x86_directmap_paddr_to_vaddr(__paddr_t paddr)
{
...
int ukarch_pte_read(vmi_instance_t vmi, __vaddr_t pt_vaddr, unsigned int lvl,
...
int pg_pt_walk(vmi_instance_t vmi,  __vaddr_t *pt_vaddr,
			     ...
			     unsigned int to_level, __pte_t *pte)
{
	...
	rc = ukarch_pte_read(vmi, *pt_vaddr, lvl, PT_Lx_IDX(vaddr, to_level), &lpte);
...
int ukplat_pt_walk(vmi_instance_t vmi, struct uk_pagetable *pt, __vaddr_t vaddr,
